// ===============================
// 1. CLOUD LOADING LOGIC
// ===============================
async function loadCloudLevel() {
    const levelId = localStorage.getItem("NEXT_LEVEL_ID");
    
    if (levelId) {
        console.log("Downloading community level:", levelId);
        
        // Ensure Firebase bridge is ready before calling
        if (typeof window.getLevelByID !== 'function') {
            console.log("Waiting for Firebase bridge...");
        ...
        if (levelData) {
            // Update the game state with Cloud Data
            currentLevelName = levelData.name;
            // Parse the blocks and set them to your engine's 'obstacles' array
            obstacles = JSON.parse(levelData.data); 
            console.log("Cloud level loaded successfully:", currentLevelName);
            
            // Refresh ghost and reset player position for the new map
            loadGhost();
            restart(); 
        }
        
        // Clear the ID so the game doesn't loop the loading logic
        localStorage.removeItem("NEXT_LEVEL_ID");
    }
}

// ===============================
// 2. ADVANCED AUDIO SYNTH ENGINE
// ===============================
...
const TILE = 50; const GRAVITY_CONST = 1.2; const MOVE_SPEED = 9;
const JUMP_POWER = 16; const PAD_POWER = 26;
const COYOTE_TIME_MAX = 100; const JUMP_BUFFER_MAX = 100;
...
document.addEventListener("keydown", e => {
    ...
    if (audioCtx.state === 'suspended') audioCtx.resume();
    if ((gameOver || gameWin) && (k === "enter" || k === "return")) restart();
    if (k === "r") restart();
...
function loadGhost() {
    ...
    if (user && allGhosts[currentLevelName] && allGhosts[currentLevelName][user]) {
        ghostPath = allGhosts[currentLevelName][user];
    } else {
        ghostPath = [];
    }
}
...
function saveStats(score) {
    ...
    // Local Leaderboard
    let lb = JSON.parse(localStorage.getItem("FD_LEADERBOARDS")) || {};
    if (!lb[currentLevelName]) lb[currentLevelName] = [];
    lb[currentLevelName].push({ username: user, time: parseFloat(score), date: new Date().toLocaleDateString() });
    lb[currentLevelName].sort((a, b) => a.time - b.time).splice(10);
    localStorage.setItem("FD_LEADERBOARDS", JSON.stringify(lb));
    
    // Cloud Leaderboard (Firebase)
    if (window.saveGlobalScore) {
        window.saveGlobalScore(currentLevelName, user, score, currentRunPath);
    }

    // Ghosts
    let ghosts = JSON.parse(localStorage.getItem("FD_GHOSTS")) || {};
    if (!ghosts[currentLevelName]) ghosts[currentLevelName] = {};
    ghosts[currentLevelName][user] = currentRunPath;
    localStorage.setItem("FD_GHOSTS", JSON.stringify(ghosts));
}

function restart() {
    player.x = 100; player.y = 100; player.velX = 0; player.velY = 0; player.gravityDir = 1;
    player.isPlane = false; player.visible = true;
...
function update() {
    deathShards.forEach(s => { s.x += s.vx; s.y += s.vy; s.vy += GRAVITY_CONST * 0.5; s.life -= 0.02; });
    ...
    if (player.isPlane) {
        ...
        if ((now - lastOnGroundTime < COYOTE_TIME_MAX) && (now - lastJumpPressTime < JUMP_BUFFER_MAX)) {
            ...
            player.onGround = false; lastJumpPressTime = 0; playSound('jump');
        }
    }
        ...
        if (rectCollision(player, t)) {
            if (t.type === "block") {
                if (player.gravityDir === 1) {
                    if (player.velY > 0) { player.y = t.y - player.height; player.onGround = true; player.velY = 0; }
                    else if (player.velY < 0) { player.y = t.y + TILE; player.velY = 0; }
                } else {
                    if (player.velY < 0) { player.y = t.y + TILE; player.onGround = true; player.velY = 0; }
                ...
                playSound('win'); saveStats(finalTime);
            }
    ...
    // Trail & Ghost Data
    currentRunPath.push({x: player.x, y: player.y});
...
function draw() {
    ...
    for (let t of obstacles) {
        ...
        if (t.type === "spike") {
            ctx.beginPath(); ctx.moveTo(t.x, t.y+TILE); ctx.lineTo(t.x+25, t.y); ctx.lineTo(t.x+50, t.y+TILE); ctx.fill();
...
window.onload = async () => {
    loadGhost();
    await loadCloudLevel(); // Check if we are playing a community map
    loop(); // Kick off the main game loop
};